...
from qgis.PyQt.QtCore import Qt, QLocale, QSettings, QVariant
...
# ---------------------------------- Qt compat: WA_DeleteOnClose (PyQt5 vs PyQt6) ----------------------------------
def _wa_delete_on_close():
    """
    PyQt5: Qt.WA_DeleteOnClose
    PyQt6: Qt.WidgetAttribute.WA_DeleteOnClose
    """
    try:
...
class PrecisionZonesPlugin:
    ...
    def run(self):
        ...
        self.dialog._plugin = self  # <- ESSENCIAL p/ export de PC e metadados

        self.dialog.setModal(False)

        # ✅ Compat Qt5/Qt6
        self.dialog.setAttribute(_wa_delete_on_close(), True)
        ...
        # Atualiza checklist de deps se existir no dialog
        if hasattr(self.dialog, "_check_deps"):
            try:
                self.dialog._check_deps()
            except Exception:
                pass
    ...
    def executar(self):
        ...
        # metadados da grade (para export PC)
        ...
        try:
            ...
            # Confirma metadados pelo raster referência (se quiser travar no resultado final)
            try:
                ref_path = primeira_saida.dataProvider().dataSourceUri().split("|")[0]
                ds_chk = gdal.Open(ref_path)
                if ds_chk:
                    self.ref_gt = ds_chk.GetGeoTransform()
                    self.ref_crs_wkt = ds_chk.GetProjection()
                    self.grid_shape = (ds_chk.RasterYSize, ds_chk.RasterXSize)
            except Exception:
                pass
    ...
    def executar_pca(self):
        try:
            ...
            # Preenche combos automaticamente
            if hasattr(self.dialog, "popular_combo_pcs"):
                self.dialog.popular_combo_pcs(len(variancias))
    ...
    def aplicar_filtro_modal(self):
        ...
        try:
            ...
            if reg.algorithmById('sagang:majorityminorityfilter'):
                saga_id = 'sagang:majorityminorityfilter'
            elif reg.algorithmById('saga:majorityfilter'):
                saga_id = 'saga:majorityfilter'
            elif reg.algorithmById('sagang:majorityfilter'):
                saga_id = 'sagang:majorityfilter'
            else:
    ...
    def carregar_csv_variancia(self):
        ...
        try:
            ...
            self.dados_amostrados = df
            QMessageBox.information(self.dialog, tr("Sucesso", "Success"),
    ...
    def exportar_relatorio_pca(self):
        if pd is None or not hasattr(self, "relatorio_pca") or not hasattr(self, "variancia_explicada"):
            ...
            return
        pasta = self.pasta_exportacao or QFileDialog.getExistingDirectory(
