...
Two modes:
  ...
  python build_extlibs_zip.py
      ...
      packages (numpy/matplotlib), and writes dist/extlibs-<tag>.zip
      (e.g. dist/extlibs-cp312-win_amd64.zip).
...
(extlibs_manager) can fetch the build matching each QGIS Python.
"""
import os
import shutil
import subprocess
...
def full_build():
    ...
    # extlibs_manager fetches), alongside the legacy extlibs.zip.
    out = os.path.join(_HERE, f"extlibs-{tag}.zip")
    build = tempfile.mkdtemp(prefix="pz_extlibs_")
    try:
